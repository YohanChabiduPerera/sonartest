...
import { GetDocumentsByCase } from "../../apis/DocumentManagement";
import { useDispatch } from "react-redux";
import { setLoading } from "../../store/loaderSlice";
import CaseTimeline from "./CaseTimeline"; // Adjust the import path as necessary

const AttorneyViewCase = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        console.log("Fetched dateCreated: ", fetchedCase.data.dateCreated);
        console.log(
          "Fetched estimated_end_date: ",
          fetchedCase.data.estimated_end_date
        );

        setCases(fetchedCase.data);
        if (
          fetchedCase.data.dateCreated ||
          fetchedCase.data.estimated_end_date
        ) {
          setIsCase(true);
  ...
  const handleAddClick = () => {
    navigate(`/attorney/case/document/add/${case_id}`);
  };
  ...
  return (
    <>
      ...
      <Box sx={{ p: 3 }}>
        {isCase && (
          <CaseTimeline
            startDate={formatDate(cases.dateCreated)}
            endDate={cases.estimated_end_date}
            documents={rows}
          />
        )}
        <Box
...
export default AttorneyViewCase;
