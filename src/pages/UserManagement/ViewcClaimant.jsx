...
import UpdateClaimantForm from "./UpdateClaimantForm";
import DeleteConfirmationDialog from "../../components/DeleteConfirmationDialog";
import { useNavigate, useParams } from "react-router-dom";
import { DeleteClaimant, GetClaimant } from "../../apis/UserManagementAPI";
...
const ViewClaimant = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        setIsMinor(claimants.age_at_accident < 18 ? true : false);
      ...
      const updatedInfo = [
        {
          ...
          guardian_data: claimants.guardian_data,
        },
      ];
  ...
  const handleOpenForm = () => {
    navigate(`/user-update/claimant/${claimant_id}`);
  };
  ...
  const handleDelete = async (event) => {
    ...
    dispatch(setLoading(true));
    try {
      const response = await DeleteClaimant(claimant_id);
            ...
            <Card>
              <CardContent>
                ...
                <Grid container spacing={2}>
                  ...
                  {guardian_data && guardian_data.length > 0 && (
                    <>
                      <Grid item xs={12}>
                      ...
                      {guardian_data.map((guardian, index) => (
                      ...
                      {cases.slice(0, 5).map((caseItem) => (
                        ...
                        >
                          ...
                          <TableCell sx={{ fontWeight: "bold" }}>
                            <Button
                              onClick={() => navigateToCase(caseItem.case_id)}
                            >
                              View Case
                            </Button>
                          </TableCell>
