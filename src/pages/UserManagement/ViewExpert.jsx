...
import UpdateExpertForm from "./UpdateExpertForm";
...
const ViewExpert = () => {
  ...
  const handleOpenForm = () => {
    navigate(`/user-update/expert/${expert_id}`);
  };
  ...
  const handleDelete = async (event) => {
    ...
    try {
      const response = await DeleteExpert(expert_id);
      // console.log("Expert  Deleted successfully:");
      dispatch(setLoading(false));
      navigate("/users");
    } catch (error) {
      // console.log("An error occurred while deleting the expert.", error);
      dispatch(setLoading(false));
    }
